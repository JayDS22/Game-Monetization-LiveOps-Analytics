...
==================================================
Robust path resolution — works locally, on Streamlit Cloud, and in any deployment.
"""
...
# Find project root by searching upward for data/processed/ directory
def find_project_root():
    # Method 1: Walk up from this script
    p = pathlib.Path(__file__).resolve().parent
    for _ in range(6):
        if (p / "data" / "processed").is_dir():
            return p
        p = p.parent
    # Method 2: Check common Streamlit Cloud paths
    for candidate in [
        pathlib.Path("/mount/src/game-monetization-liveops-analytics"),
        pathlib.Path("/mount/src/Game-Monetization-LiveOps-Analytics"),
        pathlib.Path.cwd(),
        ...
        if (candidate / "data" / "processed").is_dir():
            return candidate
    # Method 3: Search /mount/src/ for any dir with data/processed
    mount_src = pathlib.Path("/mount/src")
    if mount_src.is_dir():
        for child in mount_src.iterdir():
            if child.is_dir() and (child / "data" / "processed").is_dir():
                return child
...
def load_json(name):
    fp = PROCESSED / name
    if not fp.exists():
        st.error(f"File not found: {fp}\n\nProject root: {ROOT}\nProcessed dir exists: {PROCESSED.is_dir()}\nFiles in processed: {list(PROCESSED.iterdir()) if PROCESSED.is_dir() else 'DIR NOT FOUND'}")
        st.stop()
    with open(fp) as f:
        return json.load(f)
...
def load_csv(name):
    fp = RAW / name
    if not fp.exists():
        st.error(f"File not found: {fp}\n\nProject root: {ROOT}\nRaw dir exists: {RAW.is_dir()}\nFiles in raw: {list(RAW.iterdir()) if RAW.is_dir() else 'DIR NOT FOUND'}")
        st.stop()
...
with tab1:
    ...
    c1.metric("DAU", f"{m.get('dau_estimate',0):,}")
    c2.metric("MAU", f"{m.get('mau_estimate',0):,}")
    c3.metric("DAU/MAU", f"{m.get('dau_estimate',1)/max(m.get('mau_estimate',1),1):.1%}")
    c4.metric("Conversion", f"{m.get('conversion_rate',0):.1%}")
    c5.metric("Whale %", f"{m.get('whale_pct',0):.2f}%")
    c1,c2,c3,c4,c5 = st.columns(5)
    c1.metric("D1 Retention", f"{m.get('d1_retention',0):.1%}")
    c2.metric("D7 Retention", f"{m.get('d7_retention',0):.1%}")
    c3.metric("D30 Retention", f"{m.get('d30_retention',0):.1%}")
    c4.metric("ARPU", f"${m.get('arpu',0):.2f}")
    c5.metric("ARPPU", f"${m.get('arppu',0):.2f}")
    st.markdown("---")
    ...
    with co2:
        ...
        rw = txns.groupby(txns['transaction_date'].dt.to_period('W').astype(str))['price_usd'].sum().reset_index()
        rw.columns = ['week','revenue']
    ...
    filtered_copy['ic'] = pd.to_datetime(filtered_copy['install_date']).dt.to_period('M').astype(str)
    cr = filtered_copy.groupby('ic').agg(n=('player_id','count'), d1=('d1_retained','mean'), d7=('d7_retained','mean'), d30=('d30_retained','mean')).reset_index()
    cr = cr[cr['n'] >= 100]
    hd = cr[['ic','d1','d7','d30']].set_index('ic').T
    hd.index = ['D1','D7','D30']
...
with tab2:
    ...
    with co1:
        ...
        fig = px.line(sd, x='k', y='silhouette', markers=True, color_discrete_sequence=['#00d2ff'])
        fig.update_layout(height=300, margin=dict(t=20,b=20))
        st.plotly_chart(fig, use_container_width=True)
    with co2:
        st.subheader("Revenue Concentration (Pareto)")
        td = pd.DataFrame(seg_results['tier_revenue'])
        td = td[td['total_rev']>0].sort_values('total_rev', ascending=False)
        fig = go.Figure()
        fig.add_trace(go.Bar(x=td['player_tier'], y=td['pct_revenue'], name='% Revenue', marker_color='#ff6b6b'))
    ...
    fig = px.bar(rd, y='segment', x='count', orientation='h', color_discrete_sequence=['#7c4dff'])
    fig.update_layout(height=350, margin=dict(t=20,b=20))
    st.plotly_chart(fig, use_container_width=True)
...
with tab3:
    ...
    fig = go.Figure(go.Funnel(y=fd['stage'], x=fd['count'], textposition="inside", textinfo="value+percent initial",
        marker=dict(color=['#667eea','#764ba2','#f093fb','#f5576c','#fda085','#f6d365','#96e6a1','#72c6ef'])))
    ...
    with co1:
        ...
        dd = fd[fd['dropoff_rate']>0]
        fig = px.bar(dd, x='stage', y='dropoff_rate', color='dropoff_rate', color_continuous_scale='Reds',
                     text=dd['dropoff_rate'].apply(lambda x: f'{x:.1f}%'))
        fig.update_layout(height=350, margin=dict(t=20,b=20))
    ...
    with co2:
        ...
        fig = px.bar(td, x='bucket', y='count', color_discrete_sequence=['#4ecdc4'], text='count')
        fig.update_layout(height=350, margin=dict(t=20,b=20))
        st.plotly_chart(fig, use_container_width=True)
    ...
    fig = px.bar(cc, x='country', y='rate', color='rate', color_continuous_scale='Viridis',
                 text=cc['rate'].apply(lambda x: f'{x:.2%}'))
    fig.update_layout(height=300, margin=dict(t=20,b=20))
    st.plotly_chart(fig, use_container_width=True)
...
with tab4:
    ...
    with co2:
        ...
        for t,d in ltv_results['kaplan_meier'].items():
            fig.add_trace(go.Scatter(x=d['timeline'], y=d['survival'], mode='lines', name=f"{t} ({d['median']:.0f}d)", line=dict(color=colors.get(t,'#666'))))
        fig.update_layout(height=350, margin=dict(t=20,b=20), xaxis_title="Days", yaxis_title="Survival Prob.")
        st.plotly_chart(fig, use_container_width=True)

    st.subheader("XGBoost LTV Prediction Performance")
    xl = ltv_results['xgboost_ltv']
    co1, co2, co3 = st.columns(3)
    for i,(h,d) in enumerate(xl.items()):
        with [co1,co2,co3][i]:
            st.markdown(f"**{h.upper()}**")
            st.metric("RMSE", f"${d['rmse']:.2f}")
            st.metric("MAE", f"${d['mae']:.2f}")
            st.metric("R²", f"{d['r2']:.4f}")

    st.subheader("Feature Importance Across Horizons")
    fig = go.Figure()
    for h,d in xl.items():
        imp = d['feature_importance']
        fig.add_trace(go.Bar(name=h.upper(), x=list(imp.keys())[:8], y=list(imp.values())[:8]))
    fig.update_layout(barmode='group', height=350, margin=dict(t=20,b=20))
    fig.update_xaxes(tickangle=30)
    st.plotly_chart(fig, use_container_width=True)

    st.subheader("BG/NBD + Gamma-Gamma CLV")
    cv = ltv_results['bgnbd_clv']
    co1, co2, co3 = st.columns(3)
    co1.metric("Customers", f"{cv.get('n_customers','N/A'):,}")
    co2.metric("Avg CLV (90d)", f"${cv.get('avg_clv_90d',0):.2f}")
    co3.metric("Median CLV", f"${cv.get('median_clv_90d',0):.2f}")
...
with tab5:
    ...
    with co1:
        ...
        st.metric("Diff", f"{d1['diff']:+.4f}", delta=f"p={d1['p_value']:.4f}")
    ...
    with co2:
        ...
        st.metric("Diff", f"{d7['diff']:+.4f}", delta=f"p={d7['p_value']:.4f}")
        st.markdown(f"**{'✅ Significant' if d7['significant'] else '❌ Not Significant'}**")

    st.subheader("Bootstrap 95% CI (D7 Diff)")
    ci = ab_results['cookie_cats']['bootstrap_ci']
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=[ci['lower'],ci['upper']], y=[1,1], mode='lines', line=dict(width=8,color='#4ecdc4'), showlegend=False))
    fig.add_trace(go.Scatter(x=[ci['mean_diff']], y=[1], mode='markers', marker=dict(size=15,color='#ff6b6b'), name='Mean'))
    fig.add_vline(x=0, line_dash="dash", line_color="red")
    fig.update_layout(height=150, margin=dict(t=20,b=20), yaxis_visible=False)
    st.plotly_chart(fig, use_container_width=True)

    st.subheader("Bayesian A/B Test (D7)")
    bay = ab_results['bayesian']
    co1, co2, co3 = st.columns(3)
    co1.metric("P(Gate 30 > Gate 40)", f"{bay['prob_a_better']:.1%}")
    co2.metric("Expected Loss (A)", f"{bay['expected_loss_a']:.5f}")
    co3.metric("95% Credible Interval", f"[{bay['credible_interval'][0]:.4f}, {bay['credible_interval'][1]:.4f}]")

    st.markdown("---")
    st.subheader("Pricing A/B Test with CUPED")
    cp = ab_results['cuped_experiment']
    co1, co2, co3 = st.columns(3)
    with co1:
        st.markdown("**Standard**")
        st.metric("Diff", f"${cp['raw']['diff']:.4f}")
        st.metric("SE", f"{cp['raw']['se']:.4f}")
        st.metric("p", f"{cp['raw']['p_value']:.2e}")
    with co2:
        st.markdown("**CUPED**")
        st.metric("Diff", f"${cp['cuped']['diff']:.4f}")
        st.metric("SE", f"{cp['cuped']['se']:.4f}")
        st.metric("p", f"{cp['cuped']['p_value']:.2e}")
    with co3:
        st.markdown("**Benefit**")
        st.metric("Var Reduction", f"{cp['variance_reduction']:.1%}")
        st.metric("True Effect", f"{cp['true_effect']:.0%}")
        st.metric("SE Reduction", f"{1 - cp['cuped']['se']/cp['raw']['se']:.1%}")
...
with tab6:
    ...
    co1, co2, co3 = st.columns(3)
    co1.metric("XGBoost AUC", f"{md['xgboost']['auc']:.4f}")
    co2.metric("LightGBM AUC", f"{md['lightgbm']['auc']:.4f}")
    co3.metric("Best Model", md['best'])
    ...
    with co1:
        ...
        sd = pd.DataFrame(churn_results['shap_importance'])
        fig = px.bar(sd.head(12), y='feature', x='mean_abs_shap', orientation='h', color='mean_abs_shap', color_continuous_scale='Reds')
    ...
    with co2:
        ...
        fig = px.pie(rd, values='count', names='risk', color='risk',
                     color_discrete_map={'Low':'#4caf50','Medium':'#ff9800','High':'#f44336','Critical':'#b71c1c'}, hole=0.4)
        fig.update_layout(height=400, margin=dict(t=20,b=20))
        st.plotly_chart(fig, use_container_width=True)

    st.subheader("Confusion Matrix")
    cm = np.array(churn_results['confusion_matrix'])
    fig = go.Figure(data=go.Heatmap(z=cm, x=['Pred: Retained','Pred: Churned'], y=['Actual: Retained','Actual: Churned'],
    ...
    for r in churn_results['recommendations']:
        ...
        with st.expander(f"{ic} [{r['priority']}] {r['segment']} — {r['count']:,} players"):
            st.markdown(f"**Action:** {r['action']}")

    st.subheader("⚡ Alert Thresholds")
    al = churn_results['alert_thresholds']
    cu = churn_results['current_metrics']
